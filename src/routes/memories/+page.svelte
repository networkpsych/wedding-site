        ...
        {#if uploadStatus === 'paused'}
            <Warnings bind:this={sacVal}>
                <div class="alert-actions">
                    <button type="button" class="absolute inset-x-32 bottom-0 variant-filled-warning shadow-md rounded-lg p-1 hover:variant-filled-warning hover:opacity-75 hover:scale-110 hover:ease-in-out duration-300 font-bold" on:click={() => sacVal.$destroy()}>Close</button>
                </div>
            </Warnings>
        {:else if uploadStatus === 'unauthorized' || uploadStatus==='canceled' || uploadStatus==='unknown'}
            <Errors status={uploadStatus} bind:this={sacVal}>
                <div class="alert-actions">
                    <button type="button" class="absolute inset-x-32 bottom-0 variant-filled-error shadow-md rounded-lg p-1 hover:variant-filled-error hover:opacity-75 hover:scale-110 hover:ease-in-out duration-300 font-bold" on:click={() => sacVal.$destroy()}>Close</button>
                </div>
            </Errors>
        {/if}
    {/if}
    <div class="w-96">
        <ProgressBar value={uploadProgress} meter="bg-primary-900-50-token" track="bg-primary-300-600-token"/>

    </div>
        <button type="button" class="btn variant-soft-primary text-surface-800 font-bold text-lg" on:click={resetPage}>Reset Screen</button>
    <div>
            <div class="size-1/2">
                {#each filesArr as file, i}
                <ul class="list">
                    <li>
                        <span><img class="rounded-full size-14 space-y-2" src={images[i]} alt=""/></span>
                        <span class="text-lg font-bold flex-auto">{file.name} - {(file.size / (1024 * 1024)).toFixed(2)}MB</span>
                    </li>
                </ul>    
                {/each}
            </div>
...
{:else}
    <div class="flex flex-col items-center justify-center gap-2 px-11 pb-52">


        <div class="flex flex-col items-center">
            <div class="text-4xl font-semibold space-y-2">
    ...
    <FileDropzone 
        class="variant-glass-tertiary border-none size-1/2"
        id="file-dropzone"
        name="files" 
        accept="image/*"
        bind:files={files}
